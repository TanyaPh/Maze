...
void MazeSolver::coordinateValidation(std::pair<int, int> src,
                                      std::pair<int, int> dest) {
  if (src == dest) {
    throw std::string{
        "Incorrect coordinates: start and finish coordinates are same"};
  }
  if (src.first < 0 || src.first >= rows || src.second < 0 ||
      src.second >= cols || dest.first < 0 || dest.first >= rows ||
      dest.second < 0 || dest.second >= cols) {
    throw std::string{"Incorrect coordinates: out of range"};
  }
}

void MazeSolver::makeWave(std::vector<std::vector<int>> &wave,
  ...
  while (!q.empty()) {
    ...
    if (row - 1 > -1 && wave[row - 1][col] == -1 && !horizontal[row - 1][col]) {
      ...
      if (row - 1 == dest.first && col == dest.second)
        break;
      q.push({row - 1, col});
    }
    ...
    if (col - 1 > -1 && wave[row][col - 1] == -1 && !vertical[row][col - 1]) {
      ...
      if (row == dest.first && col - 1 == dest.second)
        break;
      q.push({row, col - 1});
    }
...
MazeSolver::makePath(std::vector<std::vector<int>> &wave,
                     std::pair<int, int> dest) {
...
std::vector<std::vector<int>> MazeSolver::findPath(std::pair<int, int> src,
                                                   std::pair<int, int> dest) {
